...
template <std::integral T>
struct tuple_size<brimir::vdp::Coord<T>> : integral_constant<size_t, 2> {};

template <size_t I, std::integral T>
struct tuple_element<I, brimir::vdp::Coord<T>> : tuple_element<I, decltype(brimir::vdp::Coord<T>::elements)> {};
