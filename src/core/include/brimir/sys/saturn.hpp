...
Defines the `brimir::Saturn` type, the primary object used for emulating a Sega Saturn system.
...
#include <brimir/hw/cart/cart.hpp>
...
struct Saturn {
    ...
    /// @tparam T the cartridge type, which must be a specialization of `brimir::cart::BaseCartridge`
    ...
    /// If no cartridge is inserted, returns a reference to `brimir::cart::NoCartridge`.
    ///
...
private:
    ...
    ///
    /// Registered as an observer of `brimir::core::Configuration::system::preferredRegionOrder`.
    ///
