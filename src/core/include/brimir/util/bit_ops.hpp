...
///
/// Optimized with BMI2 BEXTR instruction when available (Haswell 2013+).
///
...
[[nodiscard]] FORCE_INLINE constexpr T extract(T value) noexcept {
    ...
    // Optimized path: Use BMI2 BEXTR instruction (single instruction)
    // Note: Only works for runtime values, constexpr uses fallback
    #if defined(_MSC_VER) && defined(__AVX2__) && !defined(__clang__)
        if (!std::is_constant_evaluated()) {
            if constexpr (sizeof(T) == 4) {
                constexpr unsigned int length = (end - start + 1);
                constexpr unsigned int control = start | (length << 8);
                return static_cast<T>(_bextr_u32(static_cast<uint32>(value), control));
            } else if constexpr (sizeof(T) == 8) {
                constexpr unsigned int length = (end - start + 1);
                constexpr unsigned int control = start | (length << 8);
                return static_cast<T>(_bextr_u64(static_cast<uint64>(value), start, length));
            }
        }
    #endif
    
    // Fallback: Standard shift and mask (constexpr-compatible)
...
// Optimized byte swap using compiler intrinsics
#if defined(_MSC_VER)
    #include <stdlib.h>  // For _byteswap_* intrinsics
    
    template <>
    [[nodiscard]] FORCE_INLINE uint64 byte_swap<uint64>(uint64 value) noexcept {
        return _byteswap_uint64(value);  // BSWAP instruction
    }
    
    template <>
    [[nodiscard]] FORCE_INLINE uint32 byte_swap<uint32>(uint32 value) noexcept {
        return _byteswap_ulong(value);  // BSWAP instruction
    }
    
    template <>
    [[nodiscard]] FORCE_INLINE uint16 byte_swap<uint16>(uint16 value) noexcept {
        return _byteswap_ushort(value);  // ROR instruction
    }
