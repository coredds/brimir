...
or flattened. If `BRIMIR_EXTRA_INLINING` is defined, the macros behave exactly like their non-`EX` counterparts, otherwise
they do nothing.

In Debug builds, these macros have no effect in order to not disrupt the debugging experience. The inline macros can be
disabled by defining `BRIMIR_DISABLE_FORCE_INLINE`. `BRIMIR_EXTRA_INLINING` has no effect in debug builds.
...
/**
@def FORCE_INLINE_EX
@brief If `BRIMIR_EXTRA_INLINING` is defined, forces function inlining and marks the function `inline`.
...
/**
@def FLATTEN
@brief If `BRIMIR_EXTRA_INLINING` is defined, flattens the function.
...
#if !defined(NDEBUG) || defined(BRIMIR_DISABLE_FORCE_INLINE)
